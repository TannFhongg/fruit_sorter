...
class DetectionResult:
    ...
    It is captured at the moment the camera frame is read, NOT after inference.
    This ensures accurate timing for IR sensor window calculations.
    
    CRITICAL: On RPi4, inference takes 50-150ms. At 0.3 m/s belt speed,
    this would cause 4.5cm positional error if timestamp was captured
    after inference. By timestamping at capture, we maintain accuracy.
    
    timestamp_ms is used ONLY for computing delta_ms in SortController.
    ...
    confidence:   float
    # MonotonicMs — captured at frame capture time, suitable ONLY for interval measurement
    timestamp_ms: MonotonicMs
    frame_id:     int         = 0
